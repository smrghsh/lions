...
uniform vec2 uBaseUvOffset;     // patch -> base-grid uv remap (identity for the base)
uniform vec2 uBaseUvScale;
uniform float uAmbient;         // floor of the relief shading
uniform float uBrightness;      // overall drape gain
...
void main() {
    vec2 baseUv = uBaseUvOffset + vUv * uBaseUvScale;
    vec3 img = texture2D(uTextureUsgs, vUv).rgb;
    vec3 hs = texture2D(uTextureNoaa, baseUv).rgb;
    ...
    float light = uAmbient + (1.1 - uAmbient) * max(dot(n, normalize(uSun)), 0.0);
    ...
    if (uRestLayer > 0.5) {
        float p = texture2D(uRestMap, baseUv).r;
